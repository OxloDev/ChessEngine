...
function CheckBoard() {   
	...
	for(t_piece = PIECES.wP; t_piece <= PIECES.bK; ++t_piece) {
		for(t_pce_num = 0; t_pce_num < GameBoard.pceNum[t_piece]; ++t_pce_num) {
			sq120 = GameBoard.pieceList[PCEINDEX(t_piece,t_pce_num)];
...
function PrintPieceLists() {
	...
	for(piece = PIECES.wP; piece <= PIECES.bK; ++piece) {
		for(pceNum = 0; pceNum < GameBoard.pceNum[piece]; ++pceNum) {
			console.log('Piece ' + PceChar[piece] + ' on ' + PrSq( GameBoard.pieceList[PCEINDEX(piece,pceNum)] ));
		}
	}
...
function UpdateListsMaterial() {	
	...
	for(index = 0; index < 64; ++index) {
		...
		if(piece != PIECES.EMPTY) {
			...
			GameBoard.pieceList[PCEINDEX(piece,GameBoard.pceNum[piece])] = sq;
