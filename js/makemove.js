function ClearPiece(sq) {
	...
	for(index = 0; index < GameBoard.pceNum[pce]; ++index) {
		if(GameBoard.pieceList[PCEINDEX(pce,index)] == sq) {
			t_pceNum = index;
			break;
		}
	}
	...
	GameBoard.pieceList[PCEINDEX(pce, t_pceNum)] = GameBoard.pieceList[PCEINDEX(pce, GameBoard.pceNum[pce])];	
...
function AddPiece(sq, pce) {
	...
	GameBoard.material[col] += PieceVal[pce];
	GameBoard.pieceList[PCEINDEX(pce, GameBoard.pceNum[pce])] = sq;
	GameBoard.pceNum[pce]++;
...
function MovePiece(from, to) {
	...
	for(index = 0; index < GameBoard.pceNum[pce]; ++index) {
		if(GameBoard.pieceList[PCEINDEX(pce,index)] == from) {
			GameBoard.pieceList[PCEINDEX(pce,index)] = to;
			break;
		}
	}
...
function MakeMove(move) {
    ...
    if(SqAttacked(GameBoard.pieceList[PCEINDEX(Kings[side],0)], GameBoard.side))  {
         TakeMove();
    	return BOOL.FALSE;
    }
